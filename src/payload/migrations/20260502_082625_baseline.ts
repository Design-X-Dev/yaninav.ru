...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`pages\` (
  	...
  	\`slug\` text,
  	\`show_legal_divider\` integer DEFAULT false,
  ...
  await db.run(sql`CREATE TABLE \`_pages_v\` (
  	...
  	\`parent_id\` integer,
  	\`version_slug\` text,
  	\`version_show_legal_divider\` integer DEFAULT false,
  ...
  await db.run(sql`CREATE INDEX \`products_image2_idx\` ON \`products\` (\`image2_id\`);`)
  await db.run(sql`CREATE INDEX \`products_image3_idx\` ON \`products\` (\`image3_id\`);`)
  await db.run(sql`CREATE INDEX \`products_banner_image_idx\` ON \`products\` (\`banner_image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`media_video\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`source_basename\` text,
  	\`alt\` text,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`forms_emails_locales\` (
  	\`subject\` text DEFAULT 'Новое сообщение с сайта' NOT NULL,
  	\`message\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`media_video_id\`) REFERENCES \`media_video\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_media_video_id_idx\` ON \`payload_locked_documents_rels\` (\`media_video_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`memories_slides\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`image_id\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`memories_slides_order_idx\` ON \`memories_slides\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`memories_slides_parent_id_idx\` ON \`memories_slides\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`memories_slides_image_idx\` ON \`memories_slides\` (\`image_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`hero\` (
  	...
  	\`overlay_text\` text DEFAULT 'Смотреть видео',
  	\`poster_id\` integer NOT NULL,
  	\`video_mp4_id\` integer NOT NULL,
  	\`video_webm_id\` integer,
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`poster_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`video_mp4_id\`) REFERENCES \`media_video\`(\`id\`) ON UPDATE no action ON DELETE set null,
  	FOREIGN KEY (\`video_webm_id\`) REFERENCES \`media_video\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`hero_poster_idx\` ON \`hero\` (\`poster_id\`);`)
  await db.run(sql`CREATE INDEX \`hero_video_mp4_idx\` ON \`hero\` (\`video_mp4_id\`);`)
  await db.run(sql`CREATE INDEX \`hero_video_webm_idx\` ON \`hero\` (\`video_webm_id\`);`)
  await db.run(sql`CREATE TABLE \`about_features\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`icon\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`about_features_order_idx\` ON \`about_features\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`about_features_parent_id_idx\` ON \`about_features\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`about\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`enabled\` integer DEFAULT true,
  	\`heading\` text DEFAULT 'Философия бренда' NOT NULL,
  	\`lead\` text NOT NULL,
  	\`updated_at\` text,
  	\`created_at\` text
  );
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`media_video\`;`)
  ...
  await db.run(sql`DROP TABLE \`memories_slides\`;`)
  await db.run(sql`DROP TABLE \`memories\`;`)
  await db.run(sql`DROP TABLE \`hero\`;`)
  await db.run(sql`DROP TABLE \`about_features\`;`)
